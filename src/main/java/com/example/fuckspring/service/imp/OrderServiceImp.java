...
import com.example.fuckspring.converter.OrderMaster2OrderDTOConverter;
...
import com.example.fuckspring.repository.OrderDetailRepository;
import com.example.fuckspring.repository.OrderMasterRepository;
import com.example.fuckspring.service.OrderService;
import com.example.fuckspring.service.ProductService;
import com.example.fuckspring.utils.KeyUtil;
...
public class OrderServiceImp implements OrderService {
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        ...
        if (!orderDTO.getPayStatus().equals(PayStatusEnum.WAIT.getCode())){
            log.error("[支付完成订单] 订单状态不正确,orderId={},payStatus={}",orderDTO.getOrderId(),orderDTO.getPayStatus());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }
        ...
        OrderMaster updateResult = orderMasterRepository.save(orderMaster);
        if (updateResult == null){
            log.error("[订单支付完成] 支付失败,orderDTO={}",orderDTO);
